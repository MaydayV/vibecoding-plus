...
async function applyDisplayConfig(nextValues = {}, { persist = false } = {}) {
  ...
  if (persist) {
    writeUserConfigValues({
      DISPLAY_TODO_REFRESH_MS: String(values.todoRefreshMs),
      DISPLAY_CODING_REFRESH_MS: String(values.codingRefreshMs),
      DISPLAY_STYLE: values.style
    });
  }
...
function applySendTarget(nextTarget, { persist = false } = {}) {
  ...
  if (persist) {
    writeUserConfigValues({ SEND_TARGET: nextTarget });
  }
...
function applyCliCwd(target, nextCwd, { persist = false } = {}) {
  ...
  if (persist) {
    const envKey = target === "claude_code" ? "CLAUDE_CWD" : "CODEX_CWD";
    writeUserConfigValues({ [envKey]: resolvedCwd });
  }
...
async function finalizeSegment(ws, state) {
  ...
  if (getSegmentTranscriptDeliveryMode(state) === "confirm_on_device") {
  ...
  await dispatchPrompt(transcript, { textInjectionMode: getSegmentTextInjectionMode(state) });
...
async function undoPendingTranscript(ws, state) {
  ...
  const charsToUndo =
    Math.max(0, previousLength - nextLength) + (getSegmentTextInjectionMode(state) === "type_and_enter" ? 1 : 0);
    ...
    try {
      ...
      switch (message.type) {
        ...
        case "ptt_start":
          ...
          log("ptt_start", state.deviceId, state.segmentSource || "(firmware)");
        ...
        case "set_target": {
          ...
          if (config.sendTarget !== nextTarget) {
            ...
            applySendTarget(nextTarget, { persist: true });
          } else {
            emitServerReady(ws);
          }
          break;
        ...
        case "set_cli_cwd": {
          ...
          try {
            const resolvedCwd = applyCliCwd(target, nextCwd, { persist: true });
